...
namespace RentC.Controllers
{
    public class CustomerController
    {
        ...
        public string register(string customerId, string customerName, string birthDate, string zip)
        {
            ...
            if (!zip.Equals(""))
            {
                if (!customerModel.registerCustomer(new Entities.Customer(id, customerName, bdate), db))
                    return "A problem has occured when trying to register! Please try again!";
            }
            else if (!customerModel.registerCustomerWithZIP(new Entities.Customer(id, customerName, bdate, zip), db))
                    return "A problem has occured when trying to register! Please try again!";
        ...
        public string update(string customerId, string customerName, string birthDate, string zip)
        {
            ...
            if (!customerModel.verifyCustomer(id, db))
                return "You cannot update your customer profile because it doesn't exist anymore! Please create one to be able to update it!";
        ...
        {
            return customerModel.listCustomers(db);
        }
    }
}
